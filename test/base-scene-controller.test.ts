...
import BaseSceneController from '../src/scene/base/base-scene-controller'
import {EventEmitter} from 'eventemitter3';


class TestController extends BaseSceneController {
...
test('Proxy call not allowed', () => {
    expect(() => controller.syncProxy.exampleMethod(5)).toThrow();
})

test('Proxy call not allowed by host', () => {
...
test('Proxy call allowed by host', () => {
    controller.registerSyncedMethod(controller.exampleMethod, true);
...
test('Receiving packet will call method', () => {
    controller.registerSyncedMethod(controller.exampleMethod, false);
